...
func NewGenesisBlock(genesisAddr cipher.Address, genesisCoins []cipher.SHA256, timestamp uint64) (*Block, error) {
	txn := Transaction{}
	if err := txn.PushOutput(genesisAddr, genesisCoins, 1e6*uint64(len(genesisCoins))); err != nil {
		return nil, err
	}
